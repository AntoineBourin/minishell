...
int     cut_cmd_for_pipe_and_redirection_malloc(char ***ac, char *str)
{
    ...
    if (!(*ac = malloc(sizeof(char*) * 4)))
        return (1);
    while (str[i] && check_if_char_is_in_str(str[i], "<>|") != 1)
        i++;
...
void    pipe_select(int fd_out, char *str, t_env *env)
{
    ...
    /*while (check_if_char_in_str_is_in_str2("|", str))
    {
        ac = cut_cmd_for_pipe_and_redirection(&str);
        env->ope_type = 1;
        check = ft_pipe("ls", env, fd, check);
        pipe(fd);
        env->ope_type = 1;
        check = ft_pipe("grep e", env, fd, check);
        pipe(fd);
        env->ope_type = 1;
        check = ft_pipe("grep d", env, fd, check);
        pipe(fd);
        env->ope_type = 1;
        check = ft_pipe("grep i", env, fd, check);
        ft_free_ac(ac);
        ac = cut_cmd_for_pipe_and_redirection(&str);
